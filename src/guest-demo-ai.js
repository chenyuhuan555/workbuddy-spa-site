;(function initGuestDemoAi(root, factory) {
  ...
  const textResponses = Object.freeze({
    ...
    'resume-company-finder': '【模拟结果】可优先关注星河科技（演示）、远航供应链（演示）和青云健康（演示）。',
  ...
  const candidateInfo = Object.freeze({
    name: '林晓（虚构）', phone: '13800000001', email: 'demo-candidate@example.invalid',
    currentCompany: '未来智能（演示）', currentTitle: '高级产品经理', city: '深圳',
    experienceYears: 9, education: '本科（演示）',
  ...
  const arrayResponses = Object.freeze({
    'target-company-research': [
      { name: '新辰科技（演示）', industry: '企业服务', reason: '虚构推荐：业务阶段与人才经验匹配。', candidateIds: [] },
      { name: '海湾智能（演示）', industry: '人工智能', reason: '虚构推荐：存在产品和商业化岗位需求。', candidateIds: [] },
      { name: '北辰数据（演示）', industry: '数据服务', reason: '虚构推荐：可承接相邻行业人才。', candidateIds: [] },
    ],
    'match_candidates': [
  ...
  function install(target, options) {
    ...
    const call = createMockCall(options);
  ...
  return Object.freeze({ responses, createMockCall, install });
});
